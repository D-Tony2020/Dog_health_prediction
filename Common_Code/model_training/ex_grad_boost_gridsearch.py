...
for exp_name, exp_features in experiments.items():
    ...
    best_learning_rate = 0.035
    best_n_estimators = 280
    best_max_depth = 3
    best_colsample_bytree = 1.0
    best_reg_alpha = 0.329

    param_distributions = {
        'learning_rate': stats.uniform(best_learning_rate - 0.007, best_learning_rate + 0.007),
        'n_estimators': stats.randint(best_n_estimators - 20, best_n_estimators + 20),
        'max_depth': stats.randint(3, 4),
        'subsample': [0.8],
        'colsample_bytree': [0.8, 1.0],
        'reg_alpha': stats.uniform(0, 0.5),
        'reg_lambda': stats.uniform(0.35, 0.45)
    }
    ...
    if select_grid_search == "grid_search":
        ...
        grid_search = GridSearchCV(model, param_grid, scoring='roc_auc', cv=2, n_jobs=-1)
        ...
        model_filepath = '../models/extreme_gradient_boosting_best_gs.joblib'
    ...
    if select_grid_search == "random_search":
        ...
        random_search = RandomizedSearchCV(model, param_distributions, n_iter=10, scoring='roc_auc', cv=ml_stratified_kfold, n_jobs=-1, verbose=2)
        ...
        model_filepath = '../models/extreme_gradient_boosting_best_rs_v6.joblib'
        joblib.dump(best_model, model_filepath)
